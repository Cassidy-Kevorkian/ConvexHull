...
void graham_scan::convexify(graham_scan::undirected_linked_point *begin, graham_scan::undirected_linked_point *end) {
...
void graham_scan::convex_hull_parallel_rec(graham_scan::undirected_linked_point *begin, graham_scan::undirected_linked_point *end,
    ...
    if (num_points > chunk_sz) {
        ...
        std::thread t1(&convex_hull_parallel_rec, begin, mid->prev, num_points_left,
                       chunk_sz);
        std::thread t2(&convex_hull_parallel_rec, mid, end, num_points_right, chunk_sz);
        t1.join();
        t2.join();
    }
...
void graham_scan::find_min(std::vector<Point> &points, int nproc, Point &min_point) {
    ...
    for (int i = 0; i < nproc; ++i) {
        ...
        threads[i] = std::thread(graham_scan::find_min_thread, std::ref(points), start, end, std::ref(min_points[i]));
        start = new_end;
    }
    for(int i = 0; i < nproc; ++i) {
        threads[i].join();
...
void graham_scan::find_min_thread(std::vector<Point> &points, const int start, const int end, Point &min_point){
...
std::vector<Point> graham_scan::convex_hull_parallel(std::vector<Point> &points) {
	int NPROC = 10;
   // Point P = points[0];
    int num_points = points.size();
    ...
    graham_scan::undirected_linked_point *root = new graham_scan::undirected_linked_point(P); // P = points[0]
    graham_scan::undirected_linked_point *iter = root;

    for (int i = 1; i < num_points; ++i) {
        graham_scan::undirected_linked_point *helper = new graham_scan::undirected_linked_point(points[i]);
        iter->next = helper;
        helper->prev = iter;
        iter = iter->next;
    }
