...
void build_C(edge &t, std::vector<Points> &points){
    for(auto P : points){
        if(is_visible(P, t)){
            C[t].push_back(P);
        }
...
std::vector<Point> AdvancedHull(std::vector<Point> &points){
    ...
    std::vector<edge> edges = {edge(points[0], points[1]), edge(points[1], points[2]), edge(points[2], points[0])};
    for (edge: edges){
        H.insert(edge);
    }
...
void ProcessRidge(edge t1, point r, edge t2){
    return;
}